...
| 11 | #11 | local branch `codex/release-rehearsal`; final SHA recorded in PR after push | `codex/closed-beta-finalization` | `codex/release-rehearsal` | Retarget to `main` only after PR #10 lands | Full release-rehearsal suite, full PostgreSQL dataset import twice, migration rehearsal, backup/restore, Android handoff review | RBAC migration, admin policy operation changes, product UI E2E docs | Revert PR #11 merge commit | Required |
