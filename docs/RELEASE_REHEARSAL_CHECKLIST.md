...
- [!] BLOCKED Android installable preview artifact
  ...
  - persisted result: failed EAS build `f19b94bf-f646-499f-86dd-258a50d516b6` used repository root and `expo/AppEntry.js`; source now links existing EAS project from `apps/mobile`; no successful APK/AAB persisted yet
  ...
  - Android artifact evidence: `expo prebuild --platform android --no-install --clean` generated manifest with only `INTERNET` and `VIBRATE` active; storage and overlay permissions use `tools:node="remove"`; EAS project ID `30719dd8-101e-4acd-8d2a-e5880d60b721`; root cause fixed but retry artifact not recorded yet
  - blocker: EAS preview retry has not yet produced a successful APK; runtime API is explicit non-local placeholder until staging API exists
